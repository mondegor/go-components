...
type (
	...
	renderer interface {
		Render(message string, data map[string]string) (string, error)
	}
)
...
func New(useCaseTemplate template.UseCase, opts ...Option) *NoticeBuilder {
	co := &NoticeBuilder{
		...
		messageRenderer: templater.NewTemplater("{{", "}}"),
	}
...
func (co *NoticeBuilder) getSendAfter(notice entity.Notice) (time.Time, error) {
	if v := notice.Data[mrnotifier.ConfigDelayTime]; v != "" {
		...
		if err != nil {
			return time.Time{}, mr.ErrUseCaseIncorrectInternalInputData.Wrap(
				err,
...
func (co *NoticeBuilder) wrapError(err error, notice entity.Notice) error {
	return mr.ErrUseCaseOperationFailed.Wrap(err, "noticeKey", notice.Key)
}
