# GoStorage Changelog
Все изменения библиотеки GoComponents будут документироваться на этой странице.


## 2026-08-04
### Added
- Добавлены эндпоинты получения TOTP секрета и его QR кода;
- Добавлена ошибка `RetryAfterError`, несущая срок повторной попытки
  (заголовок `Retry-After` ответа `429`);
- Добавлены тесты контроллеров, usecase'ов и сервисов компонента `mrauth`;

### Changed
- Ошибки компонента `mrauth` собраны в одном месте (`mrauth/errors.go`);
- Коды ошибок приведены к именам своих сентинелов (`ErrLoginNotExists` -> `"LoginNotExists"`);
- Ошибки второго фактора переименованы по канону `2FA`
  (`Err2FAMustBeDisabledFirst` -> `ErrAuth2FAMustBeDisabledFirst`);
- Коды ошибок сопоставлены с HTTP статусами (`400`, `401`, `409`, `422`, `429`),
  контракты синхронизированы с реализацией;
- Ошибка «предъявленный клиентом идентификатор не подошёл» нормализуется в usecase,
  а не в HTTP контроллере;

### Removed
- Удалён `mrauth/repository/errors.go`, его типы перенесены в `mrauth/errors.go`;


## 2025-09-28 (v0.9.0)
### Changed
- Проведены доработки под новую версию библиотеки `go-core`;

### Removed
- Перенесены некоторые пакеты в проект `go-core`;

### Changed
- Обновлена большая часть пакетов библиотеки;
- В команде `make` перед вызовом `lint` теперь происходит форматирование кода;
- Обновлен код под новую версию `golangci`.
- Обновлены ресурсы новой версией `plantuml`.


## 2024-11-30
### Added
- Добавлен `mail.Provider` для отправки уведомлений на электронные адреса получателей;
- Добавлен `messanger.Provider` для отправки уведомлений в мессанджеры;
- Добавлена опция отложенного старта чтения данных в `mrmailer` и `mrnotifier`;

## 2024-11-17
### Added
- Добавлена очередь элементов `mrqueue` основанной на БД с возможностями:
    - захвата ограниченного кол-ва элементов для их обработки;
    - повторной обработки элементов при возникновении ошибок;
    - отложенной обработки элементов;
- Добавлен компонент `mrmailer` для массовой отправки сообщений различными провайдерами;
- Добавлен компонент `mrnotifier` для отправки персонализированных уведомлений на основе шаблонов;

### Changed
- Переработаны компоненты `mrsettings` и `mrordering`, улучшены фабрики по их созданию;

## 2024-10-11
### Added
- Подключён и настроен линтер `gci`;

## 2024-10-09
### Changed
- Заменены следующие типы:
    - `mrtype.KeyInt32` -> `uint64`;
    - `mrentity.ZeronullInt32` -> `mrentity.ZeronullUint64`;

## 2024-09-29
### Changed
- Обновлены зависимости библиотеки;

## 2024-09-28
### Added
- Добавлен пакет `mrtests/helpers` используемый в тестировании системы;

### Changed
- Переименовано `Usecase` -> `UseCase`;
- Поправлен `.editorconfig`, добавлены `*.proto`, `*.mk`;
- Удалена нулевая ёмкость у map;

## 2024-09-15
### Added
- Добавлены короткие команды в Makefile, обновлена инструкция команд.

## 2024-09-08
### Changed
- Поправлены `.env` переменные под новую версию `mrcmd`;

## 2024-08-03
### Changed
- Обновлены зависимости библиотеки;

## 2024-07-06
### Added
- Добавлена документация по описании архитектуры пакета `mrsettings`;

### Changed
- Обновлен `github.workflows`;
- Обновлена документация по командам используемых при разработке;

## 2024-06-30
### Added
- Добавлено описание по локальной разработке библиотеки;
- Добавлены комментарии для некоторых структур данных;

## 2024-06-26
### Changed
- Изменено местоположение компонентов пакетов `mrsettings` и `mrsort`;
- Фабрики для создания компонентов вынесены в отдельную директорию `factory`;

## 2024-06-24
### Changed
- Компонент `mrsettings` адаптирован для планировщика задач;
- Доработаны настройки линтеров;
- Обновлены зависимости библиотеки;

## 2024-06-16
### Added
- Добавлен компонент для доступа к произвольным настройкам;

### Changed
- Обновлена система формирования ошибок в связи с внедрением новой версии библиотеки `go-core`:
    - изменён формат создания новых ошибок;
- Подключены линтеры с их настройками (`.golangci.yaml`);
- Добавлены комментарии для публичных объектов и методов;

## 2024-03-19
### Changed
- Поправлено форматирование документации;

## 2024-03-18
### Changed
- Внедрена новая версия библиотеки `go-core`, заменены `.Caller() -> .WithCaller()`;

## 2024-03-15
### Changed
- Заменено `OrderField -> orderField` и `order_field -> order_index`;

## 2024-02-01
### Fixed
- Исправление оформления в файле README.md;

## 2024-01-30
### Changed
- Заменён устаревший интерфейс `mrcore.EventBox` на `mrsender.EventEmitter`;

## 2024-01-22
### Changed
- `FactoryErrInternalWithData` было заменено на `FactoryErrInternal.WithAttr(...)`;

## 2024-01-16
### Changed
- Переименован интерфейс `mrorderer.Component -> mrorderer.API`;

## 2023-12-10
### Changed
- Заменён `mrerr.Arg -> mrmsg.Data`;
- Доработана логика копирования объектов в `mrorderer.repository.WithMetaData`;

## 2023-12-06
### Changed
- Доработана обработка ошибок и добавлены обёртки для всех ошибок запросов;

## 2023-11-20
### Changed
- Поправлены названия переменных в сообщениях об ошибках;
- Обновлён `.editorconfig`;

## 2023-11-13
### Changed
- Переименованы некоторые переменные и функции (типа Id -> ID) в соответствии с code style языка go;
- Все файлы библиотеки были пропущены через `gofmt`;

## 2023-11-01
### Changed
- Переименован пакет `mrcom_orderer -> mrorderer`;
- В `EntityMeta` заменён метод `ForEachCond` на `Where` с логикой `SqlBuilder` из библиотеки `go-webcore`;

### Removed
- Пакет `mrcom_status` перенесён в библиотеку `go-webcore` (`ItemStatus`);

## 2023-10-08
### Added
- Добавлен статус `OnlyRemoveStatus`;
- Добавлен фильтр `ParseFilterItemStatusList`;
- Добавлен `StatusFlow`;

### Changed
- Перенос объектов "статус" из `mrcom` в `mrcom_orderer`;
- Обработка ошибок приведена к более компактному виду;

## 2023-09-20
### Changed
- Заменён адаптер `*mrpostgres.ConnAdapter` на интерфейс `mrstorage.DbConn`;
- Заменены `tabs` на пробелы в коде;

## 2023-09-16
### Add
- Добавлена модель `ChangeItemStatusRequest`;

## 2023-09-13
### Add
- Добавлен пример работы со статусами;

### Changed
- Добавлен компонент для управления порядком следования элементов;
- Добавлено описание статусов элемента и описаны варианты возможных переключений между ними;

### Fixed
- `QueryUpdate -> SqUpdate`;
