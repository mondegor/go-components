...
// UpdateScopesSettings - обновляет язык и часовой пояс в области действия всех действующих
// refresh токенов пользователя (во всех его сессиях и realm'ах): при ближайшем продлении
// сессии новая пара токенов выпускается уже с этими значениями.
//
// Обновляются только refresh токены: область действия выданных access токенов - это снимок,
// зафиксированный на момент их выпуска, и до их истечения ответы формируются по прежним
// языку и поясу.
//
// Область действия обновляется слиянием, т.к. кроме языка и пояса в ней хранятся realm
// и вид пользователя (см. entity.AuthTokenScopes), а они у сессий свои.
//
// Обновление массовое, а ноль затронутых строк - штатный случай, а не гонка: у пользователя
// может не быть ни одной открытой сессии, притом что запрос пришёл с действующим access
// токеном. Так бывает в realm'ах с подписанными токенами (access_type: jwt): access токен там
// в БД не сохраняется (см. authtoken.AuthToken.Create) и живёт до своего exp независимо
// от состояния БД, а сессию не сверяет и разбор (см. AuthTokenJWT.FetchOneByAccessToken).
// Поэтому после logout, отзыва сессии по переиспользованию refresh или вытеснения по лимиту
// сессий запрос проходит авторизацию ещё весь остаток жизни access токена - это десятки минут,
// а обновлять к тому моменту уже нечего.
//
// Ошибкой это не является: настройки сохранены в профиле, применять их просто некуда.
// Поэтому здесь ExecAffected с отброшенным счётчиком, а не Exec: Exec на нуле затронутых строк
// возвращает ErrEventStorageRecordsNotAffected, и вызывающий откатил бы транзакцию вместе
// с уже применённым обновлением профиля (см. usecase/user.ChangeSettings).
//
// Мимо сессии обновление может промахнуться и по гонке - это принятый компромисс. Окно
// приходится на середину транзакции продления (см. authtoken.AuthToken.Recreate): старый
// refresh уже переведён в "отозван" и под условие не подходит, а новая пара ещё не вставлена
// и оператору не видна, поэтому под обновление не попадает ни та строка, ни другая.
// Тогда профиль расходится с областью действия действующего refresh токена, и признак
// "настройки ещё не применены" (см. httpv1.isSettingsPending) не погаснет даже после продления
// сессии: продление копирует область действия из отзываемого токена. Гаснет он при следующей
// смене настроек либо при новом входе, где область действия собирается из профиля
// (см. authuser.Service.PrepareAuthorization).
//
// Оставлено как есть намеренно: данные не портятся и расходятся обратимо, а окно требует
// совпадения смены настроек с продлением сессии у одного пользователя. Закрыть его стоило бы
// чтения настроек профиля на каждом продлении, то есть лишнего запроса в горячем пути ради
// маловероятного случая.
func (re *AuthTokenPostgres) UpdateScopesSettings(ctx context.Context, userID uuid.UUID, langCode, timeZone string) error {
	sql := `
		UPDATE
			` + re.tableName + `
		SET
			token_scopes = token_scopes || jsonb_build_object('lang', $2::text, 'tz', $3::text)
		WHERE
			user_id = $1 AND token_type = $4 AND token_status = $5 AND expires_at > NOW();`
