...
// Тогда профиль расходится с областью действия действующего refresh токена, и продление сессии
// расхождение не снимает: продление копирует область действия из отзываемого токена. Расхождение
// устраняется при следующей смене настроек либо при новом входе, где область действия собирается
// из профиля (см. authuser.Service.PrepareAuthorization).
//
