-- --------------------------------------------------------------------------------------------------

DROP TABLE sample_schema.secure_operations_log;
DROP TABLE sample_schema.secure_operations;
DROP TABLE sample_schema.sessions_excess_queue;
DROP TABLE sample_schema.sessions_cleanup_queue;
DROP TABLE sample_schema.sessions;
DROP TABLE sample_schema.auth_tokens;
-- DROP TABLE sample_schema.visitors;
DROP TABLE sample_schema.users_activity_log;
DROP TABLE sample_schema.users_activity_stat;
DROP TABLE sample_schema.users_auth_2fa;
DROP TABLE sample_schema.users_realms;
DROP TABLE sample_schema.users;

-- --------------------------------------------------------------------------------------------------

DROP SCHEMA sample_schema;
