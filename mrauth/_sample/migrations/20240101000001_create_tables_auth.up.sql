...
INSERT INTO sample_schema.users (user_id, user_email, user_phone, lang_code, registered_ip, user_status, created_at, updated_at, deleted_at)
VALUES  ('5ad9475b-25e5-4014-9331-567a61c51f24', 'user@example.com', 9876543210, 'ru-RU', '203.0.113.7', 2/*ENABLED*/, '2024-12-09 04:39:08.482000 +03:00', '2024-12-09 04:39:08.482000 +03:00', NULL);
...
CREATE TABLE sample_schema.users_activity_stat (
    ...
    last_login_ip_str character varying(64) NOT NULL, -- remote addr + proxy addr
...
CREATE TABLE sample_schema.users_activity_log (
    ...
    user_ip_str character varying(64) NOT NULL, -- remote addr + proxy addr
...
--     visitor_hash uuid NOT NULL, -- hash(ip_str, agent)
--     visitor_ip_str character varying(64) NOT NULL, -- remote addr + proxy addr
...
CREATE TABLE sample_schema.secure_operations_log (
    ...
    client_ip int8 NOT NULL DEFAULT 0, -- remote addr
    client_ip_str character varying(64) NOT NULL DEFAULT '', -- remote addr + proxy addr
    created_at timestamp with time zone NOT NULL DEFAULT NOW()
);

CREATE INDEX ix_secure_operations_log_created_at ON sample_schema.secure_operations_log (created_at);
CREATE INDEX ix_secure_operations_log_visitor_id_created_at ON sample_schema.secure_operations_log (visitor_id, created_at);
-- TODO: client_ip хранит только IPv4; для IPv6 пишется 0, поэтому все IPv6-клиенты попадают в один
-- бакет и этот индекс (как и будущий rate-limit по IP) для них бесполезен; решить типом inet/bytea(16)
-- или индексом по client_ip_str.
CREATE INDEX ix_secure_operations_log_client_ip_created_at ON sample_schema.secure_operations_log (client_ip, created_at);
