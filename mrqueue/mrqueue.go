...
type (
	...
	Changer interface {
		ChangeProcessingToRetryByTimeout(ctx context.Context, limit int) (itemsIDs []uint64, err error)
		ChangeRetryToReady(ctx context.Context, limit int) (itemsIDs []uint64, err error)
	}

	// Cleaner - очищает очередь от обработанных/сломанных элементов.
	Cleaner interface {
		RemoveItemsWithoutAttempts(ctx context.Context, limit int) (itemsIDs []uint64, err error)
		RemoveCompletedItems(ctx context.Context, limit int) (itemsIDs []uint64, err error)
		RemoveBrokenItems(ctx context.Context, limit int) (itemsIDs []uint64, err error)
	}
	...
	Storage interface {
		...
		FetchAndUpdateStatusProcessingToRetryByTimeout(ctx context.Context, timeout time.Duration, limit int) (rowIDs []uint64, err error)
		FetchAndUpdateStatusRetryToReady(ctx context.Context, delayed time.Duration, limit int) (rowIDs []uint64, err error)
